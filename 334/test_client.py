...
def test_client_waiting_for_server(connected_client):
    ...
    time.sleep(PROCESS_SLEEP_TIME)
    assert (
        client_process.exitcode is None
    ), "Client does not wait for server and exited already."
...
def test_server_closed_connection_client_exited(connected_client):
    ...
    assert client_process.exitcode is not None, (
        "Client is still running even after server sent b" " closing message."
    )
