...
def response_map():
    return {
        key: random_value
        for key, random_value in enumerate(sample(range(1000), MAX_SLUG + 1))
    }
...
def servers_port(response_map):
    for port in range(49_152, 65_535):
        server_process = Process(
            target=async_http_test_server, args=(port, response_map)
        )
...
def test_if_async_function():
    ...
    ), "In this exercise http_client has to be coroutine function!"
